...
void __attribute__ ((interrupt)) FTM0_ISR(void)
{
  ...
  for(channel=0;channel<=NUMBER_OF_CHANNELS; channel++) /*!< scroll through the channels to check if which interrupt flag enabled */
  {
    ...
    {
      (*FTM0_Callback[channel])(FTM0_Arguments[channel]); /*!< Run the User Callback Function for channel with raised flag (in main, turning off Blue LED as requested in Lab 3 Hints) */
    }
