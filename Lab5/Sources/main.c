...
TFTMChannel FTMPacket =
{
  ...
  FTM0Callback, /*!< Setting User Callback Function NOW UNUSED */
...
/*lint -restore Enable MISRA rule (6.3) checking. */
{
  ...
  OS_Init(MODULECLK,true); //Initiates RTOS.
  ...
  OS_Start(); /*!< Starts Multithreading, assumes Thread Create functions have already be called */
...
/*! @brief Creates the Threads to be used in RTOS
 *
...
void ThreadsInit()
{
  ...
  while (OS_ThreadCreate(RTCThread, NULL, &RTCStack[THREAD_STACK_SIZE-1], RTC_PRI) != OS_NO_ERROR); //RTC Thread
  ...
  while (OS_ThreadCreate(AccelThread, NULL, &AccelStack[THREAD_STACK_SIZE-1], ACCEL_PRI) != OS_NO_ERROR); //Accel Thread
  while (OS_ThreadCreate(I2CThread, NULL, &I2CStack[THREAD_STACK_SIZE-1], I2C_PRI) != OS_NO_ERROR); //I2C Thread
}
...
void TowerInitThread(void* pData)
{
  for(;;)
  {
    ...
    if (towerModeInit && towerNumberInit && LEDs_Init())
    {
      ...
      if (RTC_Init((void*) RTCCallback , NULL) && PIT_Init(MODULECLK, (void*) &PITCallback, NULL) && (Packet_Init(BAUDRATE, MODULECLK)) && FTM_Init())
